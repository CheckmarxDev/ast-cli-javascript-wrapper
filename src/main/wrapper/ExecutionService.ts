        ...
        if(fileExtension.includes("json")){
            ...
            if (read_json.results){
                const r : CxResult[] = read_json.results.map((member:any)=>{
                    ...
                    return new CxResult(member.type,member.label,member.id,member.status,member.similarityId,member.state,member.severity,member.created,member.firstFoundAt,member.foundAt,member.firstScanId,member.description,data,member.comments,cxVulnerabilityDetails);
                });
